...
mydb=connection.MySQLConnection(user='root',password='Charitha@31',
                             host='localhost',db='snmprjdb')
# mydb=mysql.connector.connect(user='root',password='Charitha@31',
#                              host='localhost',db='snmprjdb')
app=Flask(__name__)
...
def register():
    if request.method=='POST':
        ...
        if count_email[0]==0:
            ...
            # print(gotp)
            # ap='ktoq sofs govo zqhe'
            subject='OTP verification for Simple Notes Management App'
            body=f'Use the given otp for otp verification : {gotp}'
...
def otpverify(udata):
    if request.method=='POST':
        ...
        try:
            ...
            # returns {'username':'charitha','useremail':'charithasridondapati31@gmail.com','userpassword':'123','usergender':'female','stored_otp':'Ki8Nh6'}
            ...
            if decrypted_userdata['stored_otp']==user_otp:
                ...
                cursor.execute('insert into users(username,useremail,password,gender) values(%s,%s,%s,%s)',[decrypted_userdata['username'],decrypted_userdata['useremail'],decrypted_userdata['userpassword'],decrypted_userdata['usergender']])
                mydb.commit()
                cursor.close()
...
def viewnotes(nid):
    if session.get('user'):
        ...
        notesdata=cursor.fetchone() # (1,'python','dfgfhjk','2025-09-23','anusha@codegnan.com')
